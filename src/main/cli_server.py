...
def send_info_to_subscribers(info: str, from_participant: Participant):
    ...
    if topic_obj:
        ...
        logging.info(f"The topic: {topic} does not have an object")
...
def instantiate_participant(
    ...
    # global constants_set
    # logging.info(f"Current {constants_set=}")
    logging.info(f"{participant_address}: Creating this participant Object")
    this_participant_obj = save_data_of_participant(
        participant_socket, participant_address, config_str
    )

    logging.info(
        f"Thread created for {participant_address} = {this_participant_obj.config_data.name}"
    )
...
def start_server_listening():
    ...
    # Start and Initialize Participants
    ...
    while not analysis_started:
        ...
        # Request Config Data
        logging.info(f"Request Config from {new_participant_address}")
        new_participant.send(format_msg_with_header("CONFIG"))

        # Receiving Config data
        config_str = recv_msg(new_participant)
        logging.info(
            f"Received Config string from {new_participant_address}\n {config_str}"
        )
        if config_str:
            logging.info("Instantiating this Participant")
    ...
    logging.info("start_server_listening while loop ended")


def main():
    global dds_info_object
    logging.info("Started Instantiating")
    instantiate_dds(dds_info_object)
    # Threads
    logging.info("start_server_listening thread created and started")
